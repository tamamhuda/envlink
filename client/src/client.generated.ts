...
import { reactiveApi, initAuthScheduler } from "./wrap/reactiveApi";
...
import * as WebhooksApiModule from "./generated/apis/WebhooksApi";
import * as IndexModule from "./generated/apis/index";
...
export class EnvlinkClient {
  ...
  public webhooks: ReactiveApi<WebhooksApiModule.WebhooksApi>;
  public index: ReactiveApi<IndexModule.Index>;
  ...
  constructor({ config, auth }: EnvlinkClientOptions) {
    ...
    this.account = reactiveApi(new AccountApiModule.AccountApi(configuration), auth);
    this.analytics = reactiveApi(new AnalyticsApiModule.AnalyticsApi(configuration), auth);
    this.authentication = reactiveApi(new AuthenticationApiModule.AuthenticationApi(configuration), auth);
    this.billingAddress = reactiveApi(new BillingAddressApiModule.BillingAddressApi(configuration), auth);
    this.health = reactiveApi(new HealthApiModule.HealthApi(configuration), auth);
    this.oAuth = reactiveApi(new OAuthApiModule.OAuthApi(configuration), auth);
    this.paymentMethods = reactiveApi(new PaymentMethodsApiModule.PaymentMethodsApi(configuration), auth);
    this.publicPaymentMethods = reactiveApi(new PublicPaymentMethodsApiModule.PublicPaymentMethodsApi(configuration), auth);
    this.publicSubscriptionsPlans = reactiveApi(new PublicSubscriptionsPlansApiModule.PublicSubscriptionsPlansApi(configuration), auth);
    this.publicURLs = reactiveApi(new PublicURLsApiModule.PublicURLsApi(configuration), auth);
    this.sessions = reactiveApi(new SessionsApiModule.SessionsApi(configuration), auth);
    this.subscriptions = reactiveApi(new SubscriptionsApiModule.SubscriptionsApi(configuration), auth);
    this.transactions = reactiveApi(new TransactionsApiModule.TransactionsApi(configuration), auth);
    ...
    this.webhooks = reactiveApi(new WebhooksApiModule.WebhooksApi(configuration), auth);
    this.index = reactiveApi(new IndexModule.Index(configuration), auth);
  }
}
