...
const props = defineProps<{
  browserData: Record<string, UrlAnalyticsSegmentsDeviceVisitsValue>;
}>();

// Calculate total visits
const totalVisits = computed(() => {
  return Object.values(props.browserData).reduce(
...
const displayedData = computed(() => {
  return Object.entries(props.browserData).map(([name, value]) => ({
    name,
    count: value.total || 0,
...
<template>
  <div class="box-inner-card w-full h-full min-h-[450px] p-6 flex flex-col">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-semibold">Top Browsers</h3>
    </div>
        ...
        <div class="flex-1 min-w-0 mr-4">
          <div class="flex items-center justify-between mb-1">
            <div class="flex items-center gap-2 min-w-0">
              <DashboardIcon type="browser" :name="item.name" size="w-5 h-5" />
