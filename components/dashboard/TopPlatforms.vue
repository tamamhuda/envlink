...
const props = defineProps<{
  deviceData: Record<string, UrlAnalyticsSegmentsDeviceVisitsValue>;
  osData: Record<string, UrlAnalyticsSegmentsDeviceVisitsValue>;
}>();

const activeTab = ref<"platforms" | "os">("platforms");

// Calculate total visits
const totalVisits = computed(() => {
  const data =
    activeTab.value === "platforms" ? props.deviceData : props.osData;
  return Object.values(data).reduce((sum, item) => sum + (item.total || 0), 0);
});
...
<template>
  <div class="box-inner-card w-full h-full min-h-[450px] p-6 flex flex-col">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-semibold">Top Platforms</h3>
        ...
        <div class="flex-1 min-w-0 mr-4">
          <div class="flex items-center justify-between mb-1">
            <div class="flex items-center gap-2 min-w-0">
              <DashboardIcon
                :type="activeTab === 'platforms' ? 'platform' : 'platform-os'"
                :name="item.name"
                size="w-5 h-5"
              />
