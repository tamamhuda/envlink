  ...
  <NuxtLayout name="authenticated">
    <div class="flex h-full bg-[var(--bg-color)] text-[var(--text-color)]">
      ...
      <aside
        v-if="!isUpgradePage"
        :class="[
          'fixed inset-y-20 md:inset-10 left-0 z-40 p-4 w-72  flex-shrink-0 overflow-y-auto transition-transform duration-300 ease-in-out',
      ...
      >
        <div class="flex flex-col">
          ...
          >
            <NuxtLink
              v-for="item in accountMenu.filter((i) => i.name !== 'Profile')"
              :key="item.name"
              :to="item.href"
